...
:
    ...
    //T_(this->thermo().T()),		//added
    //psi_(this->thermo().psi()),		//added
    //rho_(this->thermo().rho()),		//added
    //mu_(this->thermo().mu()),		//added
    //MMW_(this->thermo().MMW()),		//added
...
hashedWordList YSLFModel<CombThermoType>::tables()
{
	...
	//tableNames.append("T");			//added
	//tableNames.append("rho");		//added
	//tableNames.append("mu");		//added
	//tableNames.append("psi");		//added
...
void YSLFModel<CombThermoType>::correct()
{
    ...
    //const scalar RR = 1e3*physicoChemical::R.value();		//added [J/kmol/K]
    ...
    if(this->active())
    {
       ...
       forAll(Y_, i)
       {
          ...
          forAll(ZCells, cellI)
          {
        	 ...
        	 Zeta = sqrt(varZCells[cellI]/max(ZCells[cellI]*(1 - ZCells[cellI]), SMALL));
                 ...
                 //if (useProgressVariable_)   x[0] = PvCells[cellI];	//added
                 ...
                 //chiCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 3));
            	 ...
            	 //TCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 4));		//added
            	 //rhoCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 3));		//added
            	 //muCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 2));		//added
            	 //psiCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 1));		//added
        	 }
       ...
       forAll(he_.boundaryField(), patchi)   
       {
          ...
          //fvPatchScalarField& pChi = Chi_.boundaryField()[patchi];
          fvPatchScalarField& pHe = he_.boundaryField()[patchi];
          fvPatchScalarField& pSrr = Srr_.boundaryField()[patchi];	//added
          //fvPatchScalarField& pT = T_.boundaryField()[patchi];	//added
          //fvPatchScalarField& prho = rho_.boundaryField()[patchi];	//added
          //fvPatchScalarField& pmu = mu_.boundaryField()[patchi];	//added
          //fvPatchScalarField& ppsi = psi_.boundaryField()[patchi];	//added
          ...
          forAll(Y_, i)
          {
              ...
              forAll(pY , facei)
              {
             	 if (i == 0)
             	 {
                     ...
                     //if (useProgressVariable_)   x[0] = pPv[facei];	//added
                     if (useProgressVariable_)   x[0] = min(chiMax, max(0, pChi[facei]));
                     if (useMixtureFractionVariance_) x[1] = min(Zeta, 0.99);
                     x[2] = pZ[facei];
                     //if (useProgressVariable_)   x[3] = pC[facei];	//added
                     ...
                     //pT[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 4));			//added
                     //prho[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 3));			//added
                     //pmu[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 2));			//added
                     //ppsi[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 1));			//added
             	 }
